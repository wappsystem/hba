//-------------------------------------
var participant_name=function(record){ if(record.Data.ABC_ID!=undefined && record.Data.initials!=undefined) return record.Data.ABC_ID+' - '+record.Data.initials; else return record.UID;}
//-------------------------------------
...
var autocomplete_list_p=function(records){
    ...
    for(var i=0;i<records.length;i++){
        var obj={};
        if(records[i].Data.initials!= undefined ) obj.label=records[i].Data.ABC_ID+' - '+records[i].Data.initials;
...
//-------------------------------------
//auto select visit
var autocomplete_req_v={cmd:"find",table:$vm.module_list['visit-data'].Table,options:{},skip:0,limit:10}
var autocomplete_callback_v=function(items){ $("#F__ID input[name=Visit]").val(items["UID"]);}
var autocomplete_list_v=function(records){
    var items=[];
    for(var i=0;i<records.length;i++){
        var obj={};
        if(records[i].Data.Visit!= undefined ) obj.label=records[i].Data.Visit;
        else obj.label=records[i].Visit;
        obj['UID']=records[i].Visit;
        items.push(obj);
    }
    return items;
}
var wait3=function(){
    $vm.autocomplete($('#Visit__ID'),autocomplete_req_v,autocomplete_list_v,autocomplete_callback_v);
}
...
//--------------------------
    if(m.input!=undefined && m.input.participant_record!=undefined){
        ...
        $("#F__ID input[name=Visit]").val(m.input.visit);
    }
    ...
    var wait2=function(){
        ...
        $('#F__ID input[name=Visit]').prop('readonly',false);
        $('#F__ID input[name=Visit]').autocomplete( "enable" );
        ...
        if($("#F__ID input[name=Visit]").val()!=''){
            $('#F__ID input[name=Visit]').prop('readonly',true);
            $('#F__ID input[name=Visit]').autocomplete( "disable" );
        }
    }
    //--------------------------
...
m.before_submit=function(data){
    if ($("#F__ID input[name=Participant]").val()=='' || $("#F__ID input[name=Participant_uid]").val()=='' || $("#F__ID input[name=Visit]").val()==''){
        $vm.alert("Please select a participant and visit. Make sure Participant ID has a number.") 
        return false;    
    }
